...
- `src/server.js` runs the rate scraper on boot and daily at 05:45 UTC (11:30 NPT) via `node-cron`. `services/rateScraper.js` parses hamropatro's Next.js RSC payload — it is fragile against their markup changes and falls back to deriving per-gram from per-tola.
