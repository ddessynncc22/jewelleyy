...
- `src/server.js` runs the rate scraper on boot (if no rate exists for the current Nepal day) and multiple times daily (11:30, 13:00, 15:00, 17:00 NPT) via `node-cron` with `timezone: 'Asia/Kathmandu'` — the extra runs make a single failed/late scrape self-recover without a restart. `services/rateScraper.js` parses hamropatro's Next.js RSC payload (fragile against their markup changes, falls back to deriving per-gram from per-tola), buckets dates in Nepal time via `getNepalToday()`, retries 3x with backoff, and upserts per `(metalType, unit, day)` so a later scrape overwrites a stale early-morning value.
