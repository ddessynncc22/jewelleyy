...
return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2 py-2">
      <p className="text-sm text-[var(--color-text-secondary)]">
        <span className="font-medium text-[var(--color-text)] num">{from}</span>&ndash;<span className="font-medium text-[var(--color-text)] num">{to}</span> of{' '}
        <span className="font-medium text-[var(--color-text)] num">{total}</span>
      </p>
      <div className="flex items-center gap-3">
        ...
        <nav className="flex items-center gap-1">
          <button type="button" onClick={() => onPageChange?.(page - 1)} disabled={page <= 1}
            className="inline-flex items-center justify-center rounded-xl p-2 text-[var(--color-text-secondary)] hover:bg-[var(--color-ink-100)] hover:text-[var(--color-text)] disabled:opacity-30 disabled:pointer-events-none transition-colors">
